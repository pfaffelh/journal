...
* `MeasureTheory.of_continuous_comp_of_isMulSystem`, the second step of the
  ...
  The range of `x ↦ (f 1 x, …, f n x)` lies in a closed box, the box is compact,
  and the subalgebra of `C(box, ℝ)` generated by the coordinates separates points
  and contains the constants, so
  ...
  `:297` is the bundled one) approximates `φ` uniformly. The pullback along
  `x ↦ (f 1 x, …, f n x)` of that subalgebra lands in `Submodule.span ℝ (insert 1 K)`
  by the previous point, hence in `P`, and the uniform limit is in `P` by
  `of_tendstoUniformly_of_mono_lim`.
