...
/-- **The mean increment of a bounded measurable function under a Markov kernel is measurable in
the base point.**  This is the factor of `jumpApply` that does *not* carry the rate, and it is
stated on its own because the path dependent generator needs it on its own: `jumpApplyF`
multiplies it by a rate that is a functional of `(s, ω)` and no longer a function of the state, so
the state dependent product `measurable_jumpApply` is of no use there. -/
theorem measurable_integral_sub_kernel {mu : Kernel E E} [IsMarkovKernel mu] {f : E → ℝ}
    (hf : Measurable f) {C : ℝ} (hC : ∀ x, |f x| ≤ C) :
    Measurable fun x ↦ ∫ y, (f y - f x) ∂(mu x) := by
...
theorem measurable_jumpApply {lam : E → ℝ} (hlam : Measurable lam) {mu : Kernel E E}
    ...
    Measurable (jumpApply lam mu f) :=
  hlam.mul (measurable_integral_sub_kernel hf hC)
...
section StoppedMartingale

/-! ## The stopped martingale theorem in continuous time

Point 5 of Milestone 4 ends at `ProbabilityTheory.Locally`, and `Locally` asks, for every member
`τ n` of the localizing sequence, that the *stopped* process be a martingale.  **Mathlib has that
step only in discrete time**: the whole of `Probability/Martingale/OptionalStopping.lean` stands
under `{𝒢 : Filtration ℕ m0} {f : ℕ → Ω → ℝ}`, and there is no
`IsStable 𝓕 (fun Y ↦ Martingale Y 𝓕 P)` in either v4.33.1 or on `master`.  What Mathlib does
have, over an arbitrary `[LinearOrder ι] [TopologicalSpace ι] [OrderTopology ι]` and hence over
`ℝ≥0`, is optional sampling for stopping times of **countable range**
(`MeasureTheory.Martingale.stoppedValue_ae_eq_condExp_of_le_const_of_countable_range`,
`OptionalSampling.lean:90`).  This section is the passage from there to an arbitrary stopping
time.
...
close, because `𝓕 i` is *not* contained in the σ-algebra of `min i τ`: at a time `t < i` the set
`{min i τ ≤ t} = {τ ≤ t}` is not all of `Ω`, and an event of `𝓕 i` need not be `𝓕 t`-measurable.
What does close is a single auxiliary stopping time, `ρ`, equal to `min j τ` on the good part of
the test set and to the constant `i` off it (`IsStoppingTime.piecewise_of_le`); the identity
`E[Y_ρ] = E[Y_j]` for it, read against the same identity for the constant time `i`, gives the set
identity directly.
-/
...
noncomputable def dyadStop (j : ℝ≥0) (ρ : Ω → ENNReal) (n : ℕ) (ω : Ω) : ENNReal :=
  ((min j (Real.toNNReal (dyadCeil n (ρ ω).toReal)) : ℝ≥0) : ENNReal)

theorem dyadStop_le (j : ℝ≥0) (ρ : Ω → ENNReal) (n : ℕ) (ω : Ω) :
    dyadStop j ρ n ω ≤ (j : ENNReal) :=
  ENNReal.coe_le_coe.2 (min_le_left _ _)

theorem le_dyadStop {j : ℝ≥0} {ρ : Ω → ENNReal} (hρj : ∀ ω, ρ ω ≤ (j : ENNReal)) (n : ℕ) (ω : Ω) :
    ρ ω ≤ dyadStop j ρ n ω := by
  have hne : ρ ω ≠ ⊤ := ne_top_of_le_ne_top (by simp) (hρj ω)
  have hcoe : ((ρ ω).toNNReal : ENNReal) = ρ ω := ENNReal.coe_toNNReal hne
...
theorem countable_range_dyadStop (j : ℝ≥0) (ρ : Ω → ENNReal) (n : ℕ) :
    (Set.range (dyadStop j ρ n)).Countable := by
  refine Set.Countable.mono ?_ (Set.countable_range fun k : ℤ ↦
    ((min j (Real.toNNReal (((k : ℝ)) / 2 ^ n)) : ℝ≥0) : ENNReal))
  rintro _ ⟨ω, rfl⟩
  exact ⟨⌈(ρ ω).toReal * 2 ^ n⌉, rfl⟩

theorem isStoppingTime_dyadStop {𝓕 : Filtration ℝ≥0 m} {ρ : Ω → ENNReal}
    (hρ : IsStoppingTime 𝓕 ρ) {j : ℝ≥0} (hρj : ∀ ω, ρ ω ≤ (j : ENNReal)) (n : ℕ) :
    IsStoppingTime 𝓕 (dyadStop j ρ n) := by
  intro t
  show MeasurableSet[𝓕 t] {ω | dyadStop j ρ n ω ≤ (t : ENNReal)}
  rcases le_or_gt j t with hjt | htj
  · have hset : {ω | dyadStop j ρ n ω ≤ (t : ENNReal)} = Set.univ := by
      ext ω
  ...
  · set c : ℝ≥0 := Real.toNNReal (dyadFloor n (t : ℝ)) with hc
    ...
    have hset : {ω | dyadStop j ρ n ω ≤ (t : ENNReal)} = {ω | ρ ω ≤ (c : ENNReal)} := by
      ext ω
      have hne : ρ ω ≠ ⊤ := ne_top_of_le_ne_top (by simp) (hρj ω)
      ...
      rw [show ((c : ℝ≥0) : ENNReal) = ENNReal.ofReal (dyadFloor n (t : ℝ)) from rfl,
        ENNReal.le_ofReal_iff_toReal_le hne (dyadFloor_nonneg t.coe_nonneg n)]
    rw [hset]
    exact 𝓕.mono hct _ (hρ c)

theorem tendsto_dyadStop {j : ℝ≥0} {ρ : Ω → ENNReal} (hρj : ∀ ω, ρ ω ≤ (j : ENNReal)) (ω : Ω) :
...
theorem untopA_eq_toNNReal {x : ENNReal} (hx : x ≠ ⊤) : x.untopA = x.toNNReal := by
  lift x to ℝ≥0 using hx
  rfl

variable {𝓕 : Filtration ℝ≥0 m} {P : Measure Ω} [IsFiniteMeasure P]
  {Y : ℝ≥0 → Ω → ℝ}
...
theorem integral_stoppedValue_eq_of_countable_range (hY : Martingale Y 𝓕 P)
    {σ : Ω → ENNReal} (hσ : IsStoppingTime 𝓕 σ) {j : ℝ≥0} (hσj : ∀ ω, σ ω ≤ (j : ENNReal))
...
theorem martingale_stoppedProcess (hY : Martingale Y 𝓕 P) (hprog : IsStronglyProgressive 𝓕 Y)
  ...
  intro S hS _
  classical
  set B : Set Ω := S ∩ {ω | (i : ENNReal) < τ ω} with hBdef
  set A₀ : Set Ω := S ∩ {ω | τ ω ≤ (i : ENNReal)} with hA₀def
  have hgt : MeasurableSet[𝓕 i] {ω | (i : ENNReal) < τ ω} := by
    have h : {ω | (i : ENNReal) < τ ω} = {ω | τ ω ≤ (i : ENNReal)}ᶜ := by
      ext ω; simp [not_le]
    rw [h]
    exact (hτ i).compl
  ...
  set ρ : Ω → ENNReal :=
    B.piecewise (fun ω ↦ max (min (τ ω) (j : ENNReal)) (i : ENNReal)) (fun _ ↦ (i : ENNReal)) with hρdef
  ...
  have hij' : (i : ENNReal) ≤ (j : ENNReal) := by exact_mod_cast hij
  have hρj : ∀ ω, ρ ω ≤ (j : ENNReal) := fun ω ↦ by
    by_cases hω : ω ∈ B
  ...
  have hvalB : ∀ ω ∈ B, stoppedValue Y ρ ω = stoppedProcess Y τ j ω := by
    intro ω hω
    have hiτ : (i : ENNReal) < τ ω := hω.2
    have hmin : (i : ENNReal) ≤ min (τ ω) (j : ENNReal) := le_min hiτ.le hij'
  ...
  have hunion : B ∪ A₀ = S := by
    ...
    · intro h
      rcases lt_or_ge (i : ENNReal) (τ ω) with hlt | hle
      · exact Or.inl ⟨h, hlt⟩
      · exact Or.inr ⟨h, hle⟩
  have hdisj : Disjoint B A₀ := by
    refine Set.disjoint_left.2 fun ω hω hω' ↦ ?_
    have h1 : (i : ENNReal) < τ ω := hω.2
    have h2 : τ ω ≤ (i : ENNReal) := hω'.2
...
/-! ## The assembly of Point 5: from the truncated problem to the local one

Three steps stand between `martingale_stoppedProcess_mpFamily_jumpProcessE` and
`jumpProcess_isLocalMPSolution`, and this section proves two of them and names the third.

* The **identification of the two stopped test processes**.  It holds at *every* sample point and
  not only at the non explosive ones: `jumpProcessE_truncRate_eq_of_rate_le` drops the hypothesis
  that the twelfth run of the 2026-09-10 carried, by the same case distinction with which
  `jumpProcessE_eq_of_rate_eq_on_path` dropped it -- where no window contains the time, every jump
  time lies below it and the two sequences of jump times are equal outright.  The compensators
  then agree because the two integrands agree at every time **strictly** below the stopping time,
  and the one time that is left over is the upper end of the window, a null set of the clock
  (`lebesgueClock_apply_singleton`).
* The **tower step**, `martingale_of_martingale_of_stopped`: a process that is a martingale for
  one filtration, adapted to another, constant after a stopping time of the second, and whose
  events of the second filtration cut down by `{i < τ}` are events of the first, is a martingale
  for the second.  It is stated with no reference to the jump construction.
* What is **not** here is the adaptedness of the stopped test process to the *local* filtration,
  and it is the one remaining input.  It is not
  `IsStronglyProgressive.stronglyAdapted_stoppedProcess` applied to
  `isStronglyProgressive_mpFamily_jumpProcessE`, because that statement carries a bound on the
  rate and the local case has none; and the bound is not a convenience there.  For an unbounded
  rate the *unstopped* test process is not right continuous at an explosive sample point: the
  compensator over a window containing the explosion time is the integral of a function whose
  absolute value is `lam(X_u)` times a constant, and the integral of `lam(X_u)` over `[0, T_∞)` is
  `∑ ξ_k`, so the integral diverges and Bochner returns the junk value `0`.  The stopped process
  has no such defect -- below `rateTime lam n` the rate along the path is below `n` -- but that is
  a statement about the stopped process and has to be proved of it directly. -/

variable {E : Type*} [MeasurableSpace E]

/-- **The comparison with the truncated rate needs no non explosion.**  This is
`jumpProcessE_truncRate_eq` with its window hypothesis discharged by the same case distinction
that `jumpProcessE_eq_of_rate_eq_on_path` uses: where no window contains the time, *every* jump
time lies below it, `jumpTimeE_truncRate_eq` applies at every index at once, and the two step
indices are computed from one and the same sequence.

The hypothesis is read at times **strictly** below `t`, which is what the application needs: at
the hitting time itself the running supremum has reached the level, and nothing can be said about
the state occupied there. -/
theorem jumpProcessE_truncRate_eq_of_rate_le {lam : E → ℝ} {n : ℕ} {y : ℕ → E} {xi : ℕ → ℝ}
    {t : ℝ}
    (hrate : ∀ s : ℝ, 0 ≤ s → s < t →
      ENNReal.ofReal (lam (jumpProcessE lam s (y, xi))) ≤ (n : ENNReal)) :
    jumpProcessE (truncRate lam n) t (y, xi) = jumpProcessE lam t (y, xi) := by
  ...
  · push_neg at hex
    have hall : ∀ m, jumpTimeE (truncRate lam n) y xi m = jumpTimeE lam y xi m := by
      ...
      | zero => simp
      | succ k => exact hex k
    have hseq : jumpTimeE (truncRate lam n) y xi = jumpTimeE lam y xi := funext hall
    simp [jumpProcessE, stepPath, hseq]

/-- **Up to the hitting time of the level the local process is the process of the truncated
rate, at every sample point.**  This is `jumpProcessE_eq_truncRate_of_le_rateTime` with the non
explosion hypothesis removed, and the removal is what lets the identification of the two stopped
test processes be an equality of *functions* rather than an almost sure one -- which is what
`StronglyAdapted` and `IsStoppingTime`, neither of them almost sure notions, require. -/
theorem jumpProcessE_eq_truncRate_of_le_rateTime' {lam : E → ℝ} {n : ℕ}
    {ω : (ℕ → E) × (ℕ → ℝ)} {t : ℝ} (hle : ENNReal.ofReal t ≤ rateTime lam n ω) :
    jumpProcessE (truncRate lam n) t ω = jumpProcessE lam t ω := by
  obtain ⟨y, xi⟩ := ω
  refine jumpProcessE_truncRate_eq_of_rate_le fun s hs hst ↦ ?_
  refine (ofReal_lam_jumpProcessE_lt_of_lt_rateTime hs ?_).le
  exact lt_of_lt_of_le ((ENNReal.ofReal_lt_ofReal_iff_of_nonneg hs).2 hst) hle

/-- **The stopped local process and the stopped truncated process are the same function.**
`stoppedProcess_jumpProcessE_truncRate` without its non explosion hypothesis. -/
theorem stoppedProcess_jumpProcessE_truncRate' {lam : E → ℝ} {n : ℕ} (i : ℝ≥0)
    (ω : (ℕ → E) × (ℕ → ℝ)) :
    stoppedProcess (fun t : ℝ≥0 ↦ fun ω ↦ jumpProcessE (truncRate lam n) (t : ℝ) ω)
        (rateTime lam n) i ω
      = stoppedProcess (fun t : ℝ≥0 ↦ fun ω ↦ jumpProcessE lam (t : ℝ) ω)
  ...
  have hstep : ∀ lam' : E → ℝ,
    ...
    exact congrArg (fun r : ℝ≥0 ↦ jumpProcessE lam' (r : ℝ) ω) huntop
  rw [hstep, hstep]
  exact jumpProcessE_eq_truncRate_of_le_rateTime' hst

/-- **A single time is a null set of the Lebesgue clock.**  The preimage of `{s}` under
`Real.toNNReal`, intersected with the half line the clock is pushed forward from, is the single
real point `s`. -/
theorem lebesgueClock_apply_singleton (s : ℝ≥0) : lebesgueClock.q {s} = 0 := by
  show ((volume : Measure ℝ).restrict (Set.Ici (0 : ℝ))).map Real.toNNReal {s} = 0
  rw [Measure.map_apply measurable_real_toNNReal (measurableSet_singleton s),
    Measure.restrict_apply (measurable_real_toNNReal (measurableSet_singleton s))]
  have hset : (Real.toNNReal ⁻¹' {s}) ∩ Set.Ici (0 : ℝ) = {((s : ℝ≥0) : ℝ)} := by
    ext x
    simp only [Set.mem_inter_iff, Set.mem_preimage, Set.mem_singleton_iff, Set.mem_Ici]
    constructor
    · rintro ⟨h1, h2⟩
      rw [← h1, Real.coe_toNNReal x h2]
    · rintro rfl
      exact ⟨Real.toNNReal_coe, s.coe_nonneg⟩
  rw [hset, Real.volume_singleton]

variable {mu : Kernel E E} [IsMarkovKernel mu]

/-- **The two compensating windows agree below the hitting time.**  The two integrands agree at
every time *strictly* below the hitting time -- there the path is the same by
`jumpProcessE_eq_truncRate_of_le_rateTime'` and the truncation is invisible by
`truncRate_eq_of_ofReal_lt` -- and the one time that is left over is the upper end of the window,
a null set of the clock. -/
theorem setIntegral_compensatorE_truncRate_eq {lam : E → ℝ} {n : ℕ} {f : E → ℝ}
    {ω : (ℕ → E) × (ℕ → ℝ)} {s : ℝ≥0}
    (hs : ENNReal.ofReal ((s : ℝ≥0) : ℝ) ≤ rateTime lam n ω) :
    ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ s,
        jumpApply (truncRate lam n) mu f (jumpProcessE (truncRate lam n) (u : ℝ) ω)
        ∂lebesgueClock.q
      = ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ s,
        jumpApply lam mu f (jumpProcessE lam (u : ℝ) ω) ∂lebesgueClock.q := by
  ...
  have hult : ENNReal.ofReal ((u : ℝ≥0) : ℝ) < rateTime lam n ω :=
    lt_of_lt_of_le ((ENNReal.ofReal_lt_ofReal_iff_of_nonneg u.coe_nonneg).2 hlt) hs
  have hpath : jumpProcessE (truncRate lam n) ((u : ℝ≥0) : ℝ) ω
      = jumpProcessE lam ((u : ℝ≥0) : ℝ) ω :=
    jumpProcessE_eq_truncRate_of_le_rateTime' hult.le
  have hrate : truncRate lam n (jumpProcessE lam ((u : ℝ≥0) : ℝ) ω)
      = lam (jumpProcessE lam ((u : ℝ≥0) : ℝ) ω) :=
    truncRate_eq_of_ofReal_lt (ofReal_lam_jumpProcessE_lt_of_lt_rateTime u.coe_nonneg hult)
  rw [hpath]
  simp only [jumpApply, hrate]

/-- **The two stopped test processes are the same function.**  This is the first of the two steps
the sixteenth run of the 2026-09-10 left, and it is stronger than announced: no non explosion
hypothesis, so it is an equality of functions and not an almost sure one. -/
theorem stoppedProcess_mpFamily_truncRate_eq {lam : E → ℝ} {n : ℕ} {f : E → ℝ}
    (i : ℝ≥0) (ω : (ℕ → E) × (ℕ → ℝ)) :
    stoppedProcess (fun t : ℝ≥0 ↦ fun ω ↦
        f (jumpProcessE (truncRate lam n) (t : ℝ) ω)
          - ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ t,
              jumpApply (truncRate lam n) mu f (jumpProcessE (truncRate lam n) (u : ℝ) ω)
              ∂lebesgueClock.q) (rateTime lam n) i ω
      = stoppedProcess (fun t : ℝ≥0 ↦ fun ω ↦
        f (jumpProcessE lam (t : ℝ) ω)
          - ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ t,
      ...
      stoppedProcess (fun t : ℝ≥0 ↦ fun ω ↦
          f (jumpProcessE lam' (t : ℝ) ω)
            - ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ t,
                jumpApply lam' mu f (jumpProcessE lam' (u : ℝ) ω) ∂lebesgueClock.q)
          (rateTime lam n) i ω
        = f (jumpProcessE lam' ((s : ℝ≥0) : ℝ) ω)
            - ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ s,
                jumpApply lam' mu f (jumpProcessE lam' (u : ℝ) ω) ∂lebesgueClock.q :=
    fun lam' => by
      simp only [stoppedProcess]
      exact congrArg (fun r : ℝ≥0 ↦ f (jumpProcessE lam' (r : ℝ) ω)
        - ∫ u in lebesgueClock.interval Clock.Conv.optional ⊥ r,
            jumpApply lam' mu f (jumpProcessE lam' (u : ℝ) ω) ∂lebesgueClock.q) huntop
  rw [hstep, hstep, jumpProcessE_eq_truncRate_of_le_rateTime' hst,
    setIntegral_compensatorE_truncRate_eq hst]

/-! ### Two statements about martingales, with no reference to the jump construction -/

section Tower

variable {Ω : Type*} {m : MeasurableSpace Ω}

/-- **An event of `𝓕 ⊥` may be cut out of a martingale.**  This is what `Locally` asks for: its
test object is the stopped process of `fun i ↦ {ω | ⊥ < τ n ω}.indicator (Y i)`, and the indicator
is not cosmetic -- the first member of the localizing sequence of the local jump problem stops at
once (`rateTime_zero`). -/
theorem martingale_indicator_bot {𝓕 : Filtration ℝ≥0 m} {P : Measure Ω} [IsFiniteMeasure P]
    {Z : ℝ≥0 → Ω → ℝ} (hZ : Martingale Z 𝓕 P) {S : Set Ω} (hS : MeasurableSet[𝓕 ⊥] S) :
    Martingale (fun i ↦ S.indicator (Z i)) 𝓕 P := by
  have hSi : ∀ i : ℝ≥0, MeasurableSet[𝓕 i] S := fun i ↦ 𝓕.mono bot_le _ hS
  refine ⟨fun i ↦ (hZ.stronglyMeasurable i).indicator (hSi i), fun i j hij ↦ ?_⟩
  refine (condExp_indicator (hZ.integrable j) (hSi i)).trans ?_
  filter_upwards [hZ.2 i j hij] with ω hω
  by_cases hmem : ω ∈ S
  · rw [Set.indicator_of_mem hmem, Set.indicator_of_mem hmem, hω]
  · rw [Set.indicator_of_notMem hmem, Set.indicator_of_notMem hmem]

/-- **The tower step.**  A process that

* is a martingale for one filtration `𝓗`,
* is adapted to a second filtration `𝓖`,
* does not move after a stopping time `τ` of `𝓖`, and
* whose `𝓖`-events at `i`, cut down by `{i < τ}`, are `𝓗`-events at `i`,

is a martingale for `𝓖`.  The decomposition is `S = (S ∩ {i < τ}) ∪ (S ∩ {τ ≤ i})`: on the second
piece the process takes the same value at `i` and at `j` pointwise, and the first piece is where
the martingale property of `𝓗` is spent.

Nothing here refers to the jump construction; the four hypotheses are what
`stronglyAdapted_mpFamily_jumpProcessE`, `martingale_stoppedProcess_mpFamily_jumpProcessE`,
`stoppedProcess_eq_of_ge`, `isStoppingTime_rateTime` and `jumpFiltrationE_inter_lt_rateTime`
supply. -/
theorem martingale_of_martingale_of_stopped {𝓖 𝓗 : Filtration ℝ≥0 m} {P : Measure Ω}
    [IsFiniteMeasure P] {Z : ℝ≥0 → Ω → ℝ} {τ : Ω → ENNReal}
    (hZad : StronglyAdapted 𝓖 Z) (hZ : Martingale Z 𝓗 P)
    (hstop : ∀ i j : ℝ≥0, i ≤ j → ∀ ω, τ ω ≤ (i : ENNReal) → Z i ω = Z j ω)
    (hτ : ∀ i : ℝ≥0, MeasurableSet[𝓖 i] {ω | τ ω ≤ (i : ENNReal)})
    (hcut : ∀ (i : ℝ≥0) (A : Set Ω), MeasurableSet[𝓖 i] A →
      MeasurableSet[𝓗 i] (A ∩ {ω | (i : ENNReal) < τ ω})) :
    Martingale Z 𝓖 P := by
  classical
  have hint : ∀ i, Integrable (Z i) P := fun i ↦ hZ.integrable i
  refine ⟨hZad, fun i j hij ↦ ?_⟩
  refine (ae_eq_condExp_of_forall_setIntegral_eq (𝓖.le i) (hint j)
    (fun S _ _ ↦ (hint i).integrableOn) ?_ (hZad i).aestronglyMeasurable).symm
  intro S hS _
  set B : Set Ω := S ∩ {ω | (i : ENNReal) < τ ω} with hBdef
  set A₀ : Set Ω := S ∩ {ω | τ ω ≤ (i : ENNReal)} with hA₀def
  have hgt : MeasurableSet[𝓖 i] {ω | (i : ENNReal) < τ ω} := by
    have h : {ω | (i : ENNReal) < τ ω} = {ω | τ ω ≤ (i : ENNReal)}ᶜ := by
      ext ω; simp [not_le]
    rw [h]
    exact (hτ i).compl
  have hB : MeasurableSet[𝓖 i] B := hS.inter hgt
  have hA₀ : MeasurableSet[𝓖 i] A₀ := hS.inter (hτ i)
  have hA₀m : MeasurableSet A₀ := 𝓖.le i _ hA₀
  have hB' : MeasurableSet[𝓗 i] B := hcut i S hS
  have hBj : ∫ ω in B, Z j ω ∂P = ∫ ω in B, Z i ω ∂P := (hZ.setIntegral_eq hij hB').symm
  have hA₀j : ∫ ω in A₀, Z j ω ∂P = ∫ ω in A₀, Z i ω ∂P :=
  ...
  have hdisj : Disjoint B A₀ := by
    ...
    have h2 : τ ω ≤ (i : ENNReal) := hω'.2
    exact absurd h2 (not_le.2 h1)
  have hsp : ∀ k : ℝ≥0, ∫ ω in S, Z k ω ∂P
      = ∫ ω in B, Z k ω ∂P + ∫ ω in A₀, Z k ω ∂P := fun k ↦ by
    rw [← hunion, setIntegral_union hdisj hA₀m (hint k).integrableOn (hint k).integrableOn]
  rw [hsp i, hsp j, hBj, hA₀j]
...
/-! ### The criterion survives the lift of the rate

The master equation asks a **positive** rate, and the rate of a birth and death chain vanishes at
the absorbing state.  `posRate` is the lift that repairs this, and it is data the generator cannot
see (`jumpApply_posRate`).  Non explosion, however, it *can* see, and in the direction that costs:
at an absorbing state non explosion of `lam` is free, one term of the series `∑ ξ_k / lam (y k)`
being `⊤`, and the lift replaces exactly that term by the finite `ξ_k`.  So non explosion of `lam`
is not *formally* non explosion of `posRate lam`, and the transfer has to be made -- either by the
argument that the chain stands still past an absorbing state, which is the open roadmap point
`ae_mem_nonExplosiveE_posRate`, or as here.

Here the criterion crosses the lift directly, and it does so because **both** of its hypotheses are
insensitive to the lift: the generator is unchanged, and a bound `B` on a sublevel set becomes
`max B 1`.  Nothing about the trajectory is looked at, and no divergence of the waiting times is
used.  That is the whole content of the proof below. -/

/-- **The generator form of the Lyapunov criterion, for the lifted rate.**  Same hypotheses as
`ae_mem_nonExplosiveE_jumpMeasure_of_jumpApply_le` together with `habs`, the jump kernel of an
absorbing state being the Dirac measure there -- data `birthDeathKernel` already carries, and the
same hypothesis `jumpProcessE_isMPSolution_of_nonneg` takes.

At a state of vanishing rate the generator is `0` and the inequality `A f ≤ C * f` holds there for
free, so nothing at all has to be assumed about the states the criterion did not see before. -/
theorem ae_mem_nonExplosiveE_posRate_of_jumpApply_le [MeasurableSpace E] [MeasurableEq E]
    [MeasurableSingletonClass E] {lam f : E → ℝ} {C : ℝ} (hC : 0 ≤ C)
    (hlamm : Measurable lam) (hfm : Measurable f) (hf : ∀ x, 0 ≤ f x) (hlam0 : ∀ x, 0 ≤ lam x)
    (hbdd : ∀ N : ℝ, ∃ B : ℝ, ∀ x, f x ≤ N → lam x ≤ B)
    (mu : Kernel E E) [IsMarkovKernel mu]
    ...
    (hstep : ∀ z, 0 < lam z → jumpApply lam mu f z ≤ C * f z) :
    ∀ᵐ ω ∂(jumpMeasure mu nu), ω ∈ NonExplosiveE (posRate lam) := by
  refine ae_mem_nonExplosiveE_jumpMeasure_of_jumpApply_le hC (measurable_posRate hlamm) hfm hf
    (fun x ↦ (posRate_pos hlam0 x).le) ?_ mu nu hint ?_
  · intro N
    obtain ⟨B, hB⟩ := hbdd N
    refine ⟨max B 1, fun x hx ↦ ?_⟩
    by_cases h : lam x = 0
    · rw [posRate_of_eq h]
      exact le_max_right _ _
    · rw [posRate_of_ne h]
  ...
  · intro z _
    ...
    rcases (hlam0 z).lt_or_eq with h | h
    · exact hstep z h
    · rw [jumpApply, ← h, zero_mul]
      exact mul_nonneg hC (hf z)

/-- **The birth and death instance of the lifted criterion.**  Word for word the proof of
`ae_mem_nonExplosiveE_birthDeath_of_birth_le`, with the one extra hypothesis read off
`birthDeathKernel_apply`: where the total rate vanishes the kernel is the fallback branch, the
Dirac measure at the state.  The conclusion is non explosion of the **lifted** rate, and it is the
form the master equation needs. -/
theorem ae_mem_nonExplosiveE_posRate_birthDeath_of_birth_le {b d : ℕ → ℝ} {C : ℝ} (hC : 0 ≤ C)
    (hb : ∀ x, 0 ≤ b x) (hd : ∀ x, 0 ≤ d x) (hble : ∀ x : ℕ, b x ≤ C * (x + 1))
    (nu : Measure ℕ) [IsProbabilityMeasure nu]
    [IsMarkovKernel (birthDeathKernel b d)] :
    ∀ᵐ ω ∂(jumpMeasure (birthDeathKernel b d) nu),
      ω ∈ NonExplosiveE (posRate (birthDeathRate b d)) := by
  refine ae_mem_nonExplosiveE_posRate_of_jumpApply_le (lam := birthDeathRate b d)
    (f := fun x : ℕ ↦ (x : ℝ) + 1) hC
    (measurable_of_countable _) (measurable_of_countable _)
    (fun x ↦ by positivity) (fun x ↦ add_nonneg (hb x) (hd x)) ?_
    (birthDeathKernel b d) ?_ nu (fun z ↦ integrable_birthDeathKernel _ z) ?_
  · intro N
    obtain ⟨B, hB⟩ := exists_bound_of_le_nat (birthDeathRate b d) N
    exact ⟨B, fun x hx ↦ hB x (by linarith)⟩
  · intro x hx
...
/-- **The Yule process does not explode at its lifted rate.**  The constant is `β`, as for the rate
itself, and the whole proof is `β x ≤ β (x + 1)`. -/
theorem ae_mem_nonExplosiveE_posRate_yule {β : ℝ} (hβ : 0 ≤ β)
    (nu : Measure ℕ) [IsProbabilityMeasure nu]
    [IsMarkovKernel (birthDeathKernel (linearBirth β) (linearDeath 0))] :
    ∀ᵐ ω ∂(jumpMeasure (birthDeathKernel (linearBirth β) (linearDeath 0)) nu),
      ω ∈ NonExplosiveE (posRate (birthDeathRate (linearBirth β) (linearDeath 0))) := by
  refine ae_mem_nonExplosiveE_posRate_birthDeath_of_birth_le (C := β) (b := linearBirth β)
    (d := linearDeath 0) hβ
    (fun x ↦ mul_nonneg hβ (Nat.cast_nonneg x)) (fun x ↦ by simp [linearDeath]) ?_ nu
  intro x
  have : (x : ℝ) ≤ (x : ℝ) + 1 := by linarith
...
section YuleMasterEquation

/-!
## The master equation of the Yule process

`jumpMeasure_masterEquation_of_ae_nonExplosive` removed the bound on the rate and kept two
hypotheses: almost sure non explosion, and a bound on the **value of the generator** at the one
test function.  The Yule rate `β x` discharges the second on the nose
(`jumpApply_yule_indicator`), and the first is now available for its lift
(`ae_mem_nonExplosiveE_posRate_yule`).  What is left standing is `∀ x, 0 < lam x`, and the Yule
rate does not have it: `β * 0 = 0`.

**The lift is the whole step, and it is not a trick.**  `posRate lam` raises the rate to `1` at its
zeros.  Three statements say that nothing is thereby changed:

* the generator does not see it, `jumpApply_posRate`, because at a state of vanishing rate the
  generator is `0` whatever the kernel is, and the kernel of an absorbing state is prescribed to be
  the Dirac measure there -- data `birthDeathKernel` already carries;
* the paths do not see it, `jumpProcessE_posRate_eq_of_mem`, because every state the lifted chain
  jumps into from an absorbing state is that same state; and
* consequently the one dimensional laws do not see it either, `jumpLaw_posRate_eq`, which reads
  `jumpLaw (posRate lam)` as the law of `jumpProcessE lam` -- the local process of the rate itself,
...
the master equation of the Yule process in integrated form.  The index `n + 1` is not a convenience:
`jumpApply_yule_indicator_zero` says the generator annihilates `stateIndicator 0`, so the equation at
`0` is `p 0 t = p 0 0` and carries no integral. -/

variable {E : Type*}

/-- **The indicator of a state is integrable along the process.**  The process takes its values in
`E` at every time and the indicator is bounded by `1`, so this is the constant `1` dominating a
measurable function under a probability measure.  It is what turns the compensator of the master
equation into a linear combination of one dimensional laws. -/
theorem integrable_stateIndicator_jumpProcess [MeasurableSpace E] [MeasurableSingletonClass E]
    {lam : E → ℝ} (hlam : Measurable lam) (mu : Kernel E E) [IsMarkovKernel mu] (nu : Measure E)
    [IsProbabilityMeasure nu] (r : ℝ) (k : E) :
    Integrable (fun ω ↦ stateIndicator k (jumpProcess lam r ω)) (jumpMeasure mu nu) := by
  have hX : Measurable (jumpProcess lam r (E := E)) :=
    (measurable_jumpProcess hlam).comp (measurable_const.prodMk measurable_id)
  refine Integrable.mono' (integrable_const (1 : ℝ))
    ((measurable_stateIndicator k).comp hX).aestronglyMeasurable
    (Filter.Eventually.of_forall fun ω ↦ ?_)
  simpa [Real.norm_eq_abs] using abs_stateIndicator_le_one k _

/-- **A generator that is a combination of two state indicators integrates to that combination of
one dimensional laws.**  This is the step from the compensator of
`jumpMeasure_masterEquation_of_ae_nonExplosive`, an integral of `A f` along the process, to the
right hand side of the master equation, which is an expression in the laws themselves.  Nothing
about the jump construction enters; only the integrability of the two indicators does. -/
theorem integral_comp_jumpProcess_eq_sub [MeasurableSpace E] [MeasurableSingletonClass E]
    {lam : E → ℝ} (hlam : Measurable lam) {mu : Kernel E E} [IsMarkovKernel mu] (nu : Measure E)
    [IsProbabilityMeasure nu] {g : E → ℝ} {a c : ℝ} {j k : E}
    (hg : ∀ x, g x = a * stateIndicator j x - c * stateIndicator k x) (r : ℝ) :
    (∫ ω, g (jumpProcess lam r ω) ∂(jumpMeasure mu nu))
      = a * jumpLaw lam mu nu r j - c * jumpLaw lam mu nu r k := by
  have hj := integrable_stateIndicator_jumpProcess hlam mu nu r j
  have hk := integrable_stateIndicator_jumpProcess hlam mu nu r k
  calc (∫ ω, g (jumpProcess lam r ω) ∂(jumpMeasure mu nu))
      = ∫ ω, (a * stateIndicator j (jumpProcess lam r ω)
          - c * stateIndicator k (jumpProcess lam r ω)) ∂(jumpMeasure mu nu) :=
        integral_congr_ae (Filter.Eventually.of_forall fun ω ↦ hg _)
    _ = (∫ ω, a * stateIndicator j (jumpProcess lam r ω) ∂(jumpMeasure mu nu))
          - ∫ ω, c * stateIndicator k (jumpProcess lam r ω) ∂(jumpMeasure mu nu) :=
        integral_sub (hj.const_mul a) (hk.const_mul c)
    _ = a * jumpLaw lam mu nu r j - c * jumpLaw lam mu nu r k := by
        rw [integral_const_mul, integral_const_mul, jumpLaw, jumpLaw]

/-- **The one dimensional law of the lifted rate is the law of the local process.**  The master
equation is written in `jumpLaw`, which reads `jumpProcess` and therefore asks a positive rate; the
process the statement is about is `jumpProcessE lam`, which does not.  This identifies the two, and
with it `jumpLaw (posRate lam)` is not an object of its own but the law of the process of
`yule_isLocalMPSolution`.

The proof is `jumpProcessE_posRate_eq_of_mem` between the two local processes and
`jumpProcessE_eq_jumpProcess` for the lifted rate, which has the positivity that `lam` lacks.  Both
hold almost surely and for no other reason than that the waiting times are almost surely positive
and the chain almost surely does not move at an absorbing state. -/
theorem jumpLaw_posRate_eq [MeasurableSpace E] [MeasurableEq E] [MeasurableSingletonClass E]
    {lam : E → ℝ} (hlam : Measurable lam) (hlam0 : ∀ x, 0 ≤ lam x) (mu : Kernel E E)
    [IsMarkovKernel mu] (habs : ∀ x, lam x = 0 → mu x = Measure.dirac x) (nu : Measure E)
    [IsProbabilityMeasure nu]
    (hne : ∀ᵐ ω ∂(jumpMeasure mu nu), ω ∈ NonExplosiveE (posRate lam)) (t : ℝ) (k : E) :
    jumpLaw (posRate lam) mu nu t k
      = ∫ ω, stateIndicator k (jumpProcessE lam t ω) ∂(jumpMeasure mu nu) := by
  rw [jumpLaw]
  refine integral_congr_ae ?_
  filter_upwards [ae_pos_snd_jumpMeasure mu nu, ae_absorb_jumpMeasure mu hlam habs nu, hne]
    with ω h1 h2 h3
  obtain ⟨y, xi⟩ := ω
  have e1 : jumpProcessE (posRate lam) t (y, xi) = jumpProcess (posRate lam) t (y, xi) :=
    jumpProcessE_eq_jumpProcess (posRate_pos hlam0) h1 t
  rw [← e1, jumpProcessE_posRate_eq_of_mem h1 h2 h3 t]

/-- **The master equation of the Yule process.**  The rate `β x` is unbounded and vanishes at the
absorbing state, the two defects that kept `jumpMeasure_masterEquation` away from it, and both are
gone: the bound is asked of the generator at the test function and not of the rate
(`jumpApply_yule_indicator`), and the positivity is supplied by the lift, whose non explosion is
`ae_mem_nonExplosiveE_posRate_yule` and whose law is the law of the local process
(`jumpLaw_posRate_eq`).

This is the first master equation in this file at a rate that is not bounded, and it is the
statement the second of the three routes to non explosion was to rest on.  It does **not** yield
non explosion, and `tsum_jumpLaw_eq_one` says why; non explosion is a hypothesis here, discharged
by the Lyapunov criterion. -/
theorem yule_masterEquation {β : ℝ} (hβ : 0 ≤ β) (nu : Measure ℕ) [IsProbabilityMeasure nu]
    [IsMarkovKernel (birthDeathKernel (linearBirth β) (linearDeath 0))]
    (n : ℕ) {t : ℝ} (ht : 0 ≤ t) :
    jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
        (birthDeathKernel (linearBirth β) (linearDeath 0)) nu t (n + 1)
      = jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
          (birthDeathKernel (linearBirth β) (linearDeath 0)) nu 0 (n + 1)
        + ∫ r in (0:ℝ)..t,
            (β * (n : ℝ) * jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
                (birthDeathKernel (linearBirth β) (linearDeath 0)) nu r n
              - β * ((n : ℝ) + 1)
                * jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
                    (birthDeathKernel (linearBirth β) (linearDeath 0)) nu r (n + 1)) := by
  ...
  have hA : ∀ x : ℕ, jumpApply (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
    ...
    intro x
    rw [jumpApply_posRate habs]
    exact jumpApply_yule_indicator hβ n x
  ...
  have hK : ∀ x : ℕ, |jumpApply (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
    ...
    · exact hbd _ _ _ (mul_nonneg hβ (by positivity))
  have hcomp : ∀ r : ℝ,
      (∫ ω, jumpApply (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
          (birthDeathKernel (linearBirth β) (linearDeath 0)) (stateIndicator (n + 1))
          (jumpProcess (posRate (birthDeathRate (linearBirth β) (linearDeath 0))) r ω)
          ∂(jumpMeasure (birthDeathKernel (linearBirth β) (linearDeath 0)) nu))
        = β * (n : ℝ) * jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
            (birthDeathKernel (linearBirth β) (linearDeath 0)) nu r n
          - β * ((n : ℝ) + 1)
            * jumpLaw (posRate (birthDeathRate (linearBirth β) (linearDeath 0)))
                (birthDeathKernel (linearBirth β) (linearDeath 0)) nu r (n + 1) :=
    fun r ↦ integral_comp_jumpProcess_eq_sub (measurable_of_countable _) nu hA r
  rw [jumpMeasure_masterEquation_of_ae_nonExplosive
    ...
    (ae_mem_nonExplosiveE_posRate_yule hβ nu) (n + 1) hK ht]
  simp only [hcomp]
...
/-- **The cumulated rate is non negative**, from the positivity of the rate alone and at *every*
argument: below the origin the window is empty.  This is what removes the hypothesis `0 ≤ a` from
`rateInverse_le_iff`, and removing it is not cosmetic -- a measurability statement quantifies over
a whole parameter space, on which the level is a function and cannot be assumed non negative
pointwise. -/
theorem cumulativeRateF_nonneg (hpos : ∀ u, 0 < u → 0 < Λ u ω) (c : ℝ) :
    0 ≤ cumulativeRateF Λ ω c :=
  setIntegral_nonneg measurableSet_Ioc fun u hu ↦ (hpos u hu.1).le
...
**Nothing is asked of the level.**  At a negative level the inverse is `0` -- the origin already
lies in the set whose infimum it is -- and the cumulated rate is non negative there, so both sides
are true; the hypothesis `0 ≤ a` that the first version carried was spent on
`cumulativeRateF_rateInverse` and is needed only in that branch. -/
theorem rateInverse_le_iff
    (hint : ∀ r, IntervalIntegrable (fun u ↦ Λ u ω) volume 0 r)
    (hpos : ∀ u, 0 < u → 0 < Λ u ω)
    (htop : Tendsto (cumulativeRateF Λ ω) atTop atTop) {c : ℝ} (hc : 0 ≤ c) :
  ...
  · exact iff_of_true (le_trans (csInf_le ⟨0, fun r hr ↦ hr.1⟩
      ⟨le_rfl, by simpa using ha.le⟩) hc)
      (ha.le.trans (cumulativeRateF_nonneg hpos c))
...
theorem setOf_rateInverse_le
    (hint : ∀ r, IntervalIntegrable (fun u ↦ Λ u ω) volume 0 r)
    (hpos : ∀ u, 0 < u → 0 < Λ u ω)
    (htop : Tendsto (cumulativeRateF Λ ω) atTop atTop) (a c : ℝ) :
    (rateInverse Λ ω a ≤ c) ↔ (0 ≤ c ∧ a ≤ cumulativeRateF Λ ω c) := by
  by_cases hc : 0 ≤ c
  · simp only [hc, true_and]
    exact rateInverse_le_iff hint hpos htop hc
  · simp only [hc, false_and, iff_false, not_le]
    exact lt_of_lt_of_le (not_le.1 hc) (rateInverse_nonneg Λ ω a)

/-! ### The inverse is measurable in a parameter

The inverse is a definition by `sInf` and therefore reaches nothing directly.  What reaches it is
`setOf_rateInverse_le`: its sublevel sets are sublevel sets of the **cumulated rate**, and the
cumulated rate is an integral, so a parameter enters it through the integrand alone.  These two
statements are the whole measurability theory of the path dependent construction, and note what
they do *not* mention -- no filtration, no probability, no jump times: the parameter space is an
arbitrary measurable space and the sample point is a function of it. -/

/-- **The cumulated rate is measurable in a parameter**, from the joint measurability of the rate in
the parameter and the time and from nothing else.  It is Fubini's measurability half
(`StronglyMeasurable.integral_prod_right'`) against the finite measure `volume.restrict (0, c]`. -/
theorem measurable_cumulativeRateF {γ : Type*} [MeasurableSpace γ] {Λ : γ → ℝ → Ω → ℝ}
    {p : γ → Ω} (h : Measurable fun q : γ × ℝ ↦ Λ q.1 q.2 (p q.1)) (c : ℝ) :
    Measurable fun g ↦ cumulativeRateF (Λ g) (p g) c :=
  (h.stronglyMeasurable.integral_prod_right'
    (ν := (volume.restrict (Set.Ioc (0 : ℝ) c)))).measurable

/-- **The generalised inverse is measurable in a parameter.**  The level is itself a measurable
function of the parameter, which is what the jump times need: the `(n+1)`-st of them is the inverse
at the level `ξ 0 + ⋯ + ξ n`.

The three hypotheses of the inversion are asked at every parameter value, and the fourth is the
joint measurability that `measurable_cumulativeRateF` consumes.  **The level is not asked to be non
negative**, and that is the point of the generalisation of `rateInverse_le_iff`: over a parameter
space the level is a function, and the partial sums of waiting times are non negative only on the
set where the waiting times are, which is not all of the space. -/
theorem measurable_rateInverse {γ : Type*} [MeasurableSpace γ] {Λ : γ → ℝ → Ω → ℝ}
    {p : γ → Ω} {a : γ → ℝ}
    (hint : ∀ g r, IntervalIntegrable (fun u ↦ Λ g u (p g)) volume 0 r)
    (hpos : ∀ g u, 0 < u → 0 < Λ g u (p g))
    (htop : ∀ g, Tendsto (cumulativeRateF (Λ g) (p g)) atTop atTop)
    (hmeas : Measurable fun q : γ × ℝ ↦ Λ q.1 q.2 (p q.1)) (ha : Measurable a) :
    Measurable fun g ↦ rateInverse (Λ g) (p g) (a g) := by
  refine measurable_of_Iic fun c ↦ ?_
  by_cases hc : (0 : ℝ) ≤ c
  · have hset : (fun g ↦ rateInverse (Λ g) (p g) (a g)) ⁻¹' Set.Iic c
        = {g | a g ≤ cumulativeRateF (Λ g) (p g) c} := by
      ext g
      simpa [hc] using setOf_rateInverse_le (hint g) (hpos g) (htop g) (a g) c
    rw [hset]
    exact measurableSet_le ha (measurable_cumulativeRateF hmeas c)
  · have hset : (fun g ↦ rateInverse (Λ g) (p g) (a g)) ⁻¹' Set.Iic c = (∅ : Set γ) := by
      ext g
      simp only [Set.mem_preimage, Set.mem_Iic, Set.mem_empty_iff_false, iff_false, not_le]
      exact lt_of_lt_of_le (not_le.1 hc) (rateInverse_nonneg _ _ _)
    rw [hset]
    exact MeasurableSet.empty
...
/-! ### The filtration of a point process, and why it cannot be the filtration of the path

`not_isStoppingTime_min_jumpTimeE` is the obstruction of the state dependent case, and it is not an
artefact of that case: the jump times are **not** stopping times for the natural filtration of the
process, because a constant chain gives a constant path and the waiting times leave no trace in it.
The repair there was to localize with `rateTime`, a functional of the path.  Here the repair is a
different one, and it is the one `ex:hawkes` prescribes itself: the Hawkes rate reads the **counting
measure** of the past events, so the filtration it is predictable for is the one that counting
measure generates, and a counting process sees its own jump times by construction.

`jumpRecord` is that counting process in the form `naturalFiltration` accepts -- the boolean family
`n ↦ (T n ≤ t)`, which is the counting measure of `T` on `[0, t]` read off one jump at a time.
Nothing whatever is asked of the family `T` but the measurability of each `T n`: not monotonicity,
not non explosion, not positivity.  The filtration is generated by the record **and** the path
(`jumpState`), because the record alone does not see the marks.

At this level `isStoppingTime_jumpTime` is bookkeeping; what it is worth is said in the Hawkes
instance below, where `hawkesPathFiltration_le_hawkesFiltration` and
`not_isStoppingTime_hawkesJumpTime_pathFiltration` together say that the enlargement is **needed**
and not merely convenient. -/

section PointFiltration

variable [MeasurableSpace Ω] {E : Type*} [MeasurableSpace E]
variable {T : Ω → ℕ → ℝ} {y : Ω → ℕ → E}

/-- **The record of which jumps have happened by the time `i`.**  This is the counting process of
the family `T`, coordinatewise: the `n`-th entry says whether the `n`-th jump has already
occurred. -/
noncomputable def jumpRecord (T : Ω → ℕ → ℝ) (i : ℝ≥0) (ω : Ω) : ℕ → Bool :=
  fun n ↦ decide (T ω n ≤ (i : ℝ))

theorem measurable_jumpRecord (hT : ∀ n, Measurable fun ω ↦ T ω n) (i : ℝ≥0) :
    Measurable (jumpRecord T i) := by
  refine measurable_pi_lambda _ fun n ↦ measurable_to_bool ?_
  have hset : (fun ω ↦ jumpRecord T i ω n) ⁻¹' {true} = {ω | T ω n ≤ (i : ℝ)} := by
    ext ω
    simp [jumpRecord]
  rw [hset]
  exact measurableSet_le (hT n) measurable_const

/-- **The state of a point process at the time `i`**: which jumps have happened, and where the path
is.  The second component is what the state dependent construction calls the process; the first is
what it is missing. -/
noncomputable def jumpState (T : Ω → ℕ → ℝ) (y : Ω → ℕ → E) (i : ℝ≥0) (ω : Ω) :
    (ℕ → Bool) × E :=
  (jumpRecord T i ω, stepPath (T ω) (y ω) (i : ℝ))

theorem measurable_jumpState (hT : ∀ n, Measurable fun ω ↦ T ω n)
    (hy : ∀ n, Measurable fun ω ↦ y ω n) (i : ℝ≥0) :
    Measurable (jumpState T y i) :=
  (measurable_jumpRecord hT i).prodMk
    (measurable_stepPath_comp (u := fun _ : Ω ↦ (i : ℝ)) measurable_const hT hy)

/-- **The natural filtration of a point process**, indexed by `ℝ≥0` as `jumpFiltration` and
`jumpFiltrationE` are, because `mpFamily` needs `[OrderBot ι]`. -/
noncomputable def pointFiltration (T : Ω → ℕ → ℝ) (y : Ω → ℕ → E)
    (hT : ∀ n, Measurable fun ω ↦ T ω n) (hy : ∀ n, Measurable fun ω ↦ y ω n) :
    Filtration ℝ≥0 (inferInstance : MeasurableSpace Ω) :=
  naturalFiltration (fun i : ℝ≥0 ↦ jumpState T y i) (measurable_jumpState hT hy)

/-- The set on which the `n`-th jump has happened, read in the state space of the record. -/
theorem measurableSet_record (n : ℕ) :
    MeasurableSet {p : (ℕ → Bool) × E | p.1 n = true} := by
  have hpre : {p : (ℕ → Bool) × E | p.1 n = true}
      = (fun p : (ℕ → Bool) × E ↦ p.1 n) ⁻¹' {true} := rfl
  rw [hpre]
  exact ((measurable_pi_apply n).comp measurable_fst) (measurableSet_singleton true)

/-- **The jump times of a point process are stopping times for its own filtration.**

The level set `{ofReal (T n) ≤ i}` is the `n`-th coordinate of the record at `i` and nothing else,
so the statement is the definition of `jumpRecord` read back.  **No hypothesis on the family beyond
measurability**, and in particular none of positivity: `ENNReal.ofReal` sends a negative jump time
to `0`, and a negative jump time is below every `i : ℝ≥0` in the real order as well, so the two
sides agree there too. -/
theorem isStoppingTime_jumpTime (hT : ∀ n, Measurable fun ω ↦ T ω n)
    (hy : ∀ n, Measurable fun ω ↦ y ω n) (n : ℕ) :
    IsStoppingTime (pointFiltration T y hT hy) fun ω ↦ ENNReal.ofReal (T ω n) := by
  intro i
  have key : ∀ ω : Ω, ENNReal.ofReal (T ω n) ≤ ((i : ℝ≥0) : ENNReal) ↔ T ω n ≤ (i : ℝ) := by
    intro ω
    rw [← ENNReal.ofReal_coe_nnreal, ENNReal.ofReal_le_ofReal_iff i.coe_nonneg]
  refine MeasurableSet.congr
    (s := jumpState T y i ⁻¹' {p : (ℕ → Bool) × E | p.1 n = true})
    ((measurable_naturalFiltration (X := fun i : ℝ≥0 ↦ jumpState T y i)
      (measurable_jumpState hT hy) le_rfl) (measurableSet_record n)) ?_
  ext ω
  simp only [Set.mem_preimage, Set.mem_ofPred_eq, jumpState, jumpRecord, decide_eq_true_eq]
  exact (key ω).symm

/-- **The path of a point process is adapted to its own filtration**, the second component of the
state being the path.  Stated as measurability and not as `Adapted`, because `Adapted` asks for
`StronglyMeasurable`, which over a bare `[MeasurableSpace E]` cannot be stated -- the same reason
`naturalFiltration` exists at all. -/
theorem measurable_stepPath_pointFiltration (hT : ∀ n, Measurable fun ω ↦ T ω n)
    (hy : ∀ n, Measurable fun ω ↦ y ω n) (i : ℝ≥0) :
    Measurable[pointFiltration T y hT hy i] fun ω ↦ stepPath (T ω) (y ω) (i : ℝ) :=
...
/-- **The frozen rate is locally integrable, and this discharges the hypothesis the whole recursion
carried.**  Every statement about the Hawkes jump times above and below asks for
`∀ m r, IntervalIntegrable (fun u ↦ hawkesFrozen ν φ T m u ω) volume 0 r`, and it is asked of a
family `T` that the recursion itself produces, so at first sight it is circular.  It is not: the
frozen rate is a constant plus a **finite** sum of translates of `φ`, so local integrability of
every translate of `φ` gives it outright, whatever the family.

**Which hypothesis on `φ` this is, and which it is not.**  It is local integrability of `φ`
*carried along the translation*, and for the kernel of `ex:hawkes` -- supported on the half line,
non negative, of finite mass on every window -- it is what "locally integrable kernel" says.  It is
**not** the non explosion: that is the local integrability of the **self referential** rate
`hawkesSelfRate`, which asks the jump times to exhaust the half line, and no bound on `φ` alone
gives it. -/
theorem intervalIntegrable_hawkesFrozen
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r)
    (T : ℕ → ℝ) (n : ℕ) (ω : Ω) (r : ℝ) :
    IntervalIntegrable (fun u ↦ hawkesFrozen ν φ T n u ω) volume 0 r := by
  have hsum : IntervalIntegrable (fun u ↦ ∑ k ∈ Finset.Ico 1 n, φ (u - T k)) volume 0 r := by
    have h := IntervalIntegrable.sum (μ := volume) (a := 0) (b := r) (Finset.Ico 1 n)
      (f := fun k u ↦ φ (u - T k)) fun k _ ↦ hφint (T k) r
    have heq : (∑ k ∈ Finset.Ico 1 n, fun u ↦ φ (u - T k))
        = fun u ↦ ∑ k ∈ Finset.Ico 1 n, φ (u - T k) := by
      funext u
      simp
    rwa [heq] at h
  simpa [hawkesFrozen] using intervalIntegrable_const.add hsum
...
/-! #### The jump times do not read the sample point, and are measurable in the waiting times

`hawkesJumpTime` carries a sample point `ω` because `rateInverse` does, and `rateInverse` carries
one because a path dependent rate is a functional of it.  The Hawkes rate, however, is a functional
of the **event times** and not of the marks, and the event times are built from the waiting times
alone -- so the argument `ω` of `hawkesFrozen` is a dummy, and `hawkesJumpTime ν φ xi ω n` does not
depend on `ω` in any way.  `hawkesJumpTime_sample_congr` says so across *different* sample spaces,
which is the form the measurability uses: the jump times of the process on `(ℕ → E) × (ℕ → ℝ)`
factor through `Prod.snd`.

**That is what makes the measurability of this construction cheap, and it is a statement about
Hawkes and not about path dependence.**  A rate that read the marks of the chain -- a marked Hawkes
process, where the excitation depends on what kind of event occurred -- would have jump times that
genuinely depend on the first coordinate, and the recursion would have to be carried through it.
The present measurability proof would still go through, because `measurable_rateInverse` asks
nothing about which coordinates the rate reads; what would change is the joint measurability that
has to be fed to it. -/

/-- The inverse of a frozen rate does not depend on the sample point, because the frozen rate does
not: `hawkesFrozen` discards its last argument.  The two sample spaces need not even be the same
type. -/
theorem rateInverse_hawkesFrozen_sample {Ω' : Type*} (ν : ℝ) (φ : ℝ → ℝ) (T : ℕ → ℝ) (m : ℕ)
    (ω : Ω) (ω' : Ω') (a : ℝ) :
    rateInverse (hawkesFrozen ν φ T m) ω a = rateInverse (hawkesFrozen ν φ T m) ω' a := rfl

theorem hawkesStep_sample_congr {Ω' : Type*} (ν : ℝ) (φ : ℝ → ℝ) (xi : ℕ → ℝ)
    (ω : Ω) (ω' : Ω') (n : ℕ) : hawkesStep ν φ xi ω n = hawkesStep ν φ xi ω' n := by
  induction n with
  | zero => rfl
  | succ n ih =>
      funext m
      simp only [hawkesStep, ih]
      split
      · rfl
      · exact rateInverse_hawkesFrozen_sample ν φ _ (n + 1) ω ω' _

/-- **The Hawkes jump times are a function of the waiting times alone.** -/
theorem hawkesJumpTime_sample_congr {Ω' : Type*} (ν : ℝ) (φ : ℝ → ℝ) (xi : ℕ → ℝ)
    (ω : Ω) (ω' : Ω') (n : ℕ) :
    hawkesJumpTime ν φ xi ω n = hawkesJumpTime ν φ xi ω' n := by
  rw [hawkesJumpTime, hawkesJumpTime, hawkesStep_sample_congr ν φ xi ω ω' n]

/-- **The Hawkes jump times are measurable in the waiting times**, every stage of them, and the
induction is the recursion that defined them.

The statement is the one an induction can carry: at the stage `n + 1` the frozen rate reads the
jump times at the indices `1, …, n`, so what is needed is the measurability of *all* earlier stages
and not only of the immediately preceding one.

**No hypothesis beyond the three that the inverse needs, and the measurability of `φ`.**  In
particular there is no bound on `φ` and no condition on its mass -- the same shape as
`jumpProcess_isLocalMPSolution`, which asks nothing of `lam` but its measurability.  Nor is the
positivity of the waiting times asked: a parameter space cannot carry it, and
`measurable_rateInverse` was generalised precisely so that it need not. -/
theorem measurable_hawkesJumpTime (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x) (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (ω : Ω) :
    ∀ n, ∀ m ≤ n, Measurable fun xi : ℕ → ℝ ↦ hawkesJumpTime ν φ xi ω m := by
  intro n
  induction n with
  | zero =>
      intro m hm
  ...
  | succ n ih =>
      intro m hm
      rcases Nat.lt_succ_iff_lt_or_eq.1 (Nat.lt_succ_of_le hm) with h | h
      · exact ih m (Nat.lt_succ_iff.1 h)
      · subst h
        have hrw : (fun xi : ℕ → ℝ ↦ hawkesJumpTime ν φ xi ω (n + 1))
            = fun xi : ℕ → ℝ ↦ rateInverse
              (hawkesFrozen ν φ (hawkesJumpTime ν φ xi ω) (n + 1)) ω
              (∑ k ∈ Finset.range (n + 1), xi k) :=
          funext fun xi ↦ hawkesJumpTime_succ ν φ xi ω n
        rw [hrw]
        refine measurable_rateInverse
          (Λ := fun xi ↦ hawkesFrozen ν φ (hawkesJumpTime ν φ xi ω) (n + 1))
          (p := fun _ ↦ ω)
          (fun g r ↦ intervalIntegrable_hawkesFrozen hφint _ _ _ r)
          (fun g u _ ↦ hν.trans_le (le_hawkesFrozen hφ _ _ u ω))
          (fun g ↦ tendsto_cumulativeRateF_atTop_of_le
            (fun r ↦ intervalIntegrable_hawkesFrozen hφint _ _ _ r) hν
            (fun u _ ↦ le_hawkesFrozen hφ _ _ u ω)) ?_ ?_
        · have hjoint : Measurable fun q : (ℕ → ℝ) × ℝ ↦
              ν + ∑ k ∈ Finset.Ico 1 (n + 1), φ (q.2 - hawkesJumpTime ν φ q.1 ω k) := by
            refine measurable_const.add (Finset.measurable_sum _ fun k hk ↦ ?_)
            refine hφm.comp (measurable_snd.sub ?_)
            exact (ih k (Nat.lt_succ_iff.1 (Finset.mem_Ico.1 hk).2)).comp measurable_fst
          simpa [hawkesFrozen] using hjoint
...
/-- **The jump times increase strictly, with the carried hypothesis discharged.**  This is
`strictMono_hawkesJumpTime` with `intervalIntegrable_hawkesFrozen` substituted for its integrability
hypothesis, and it is the form in which the statement should be read: what the increase of the jump
times rests on is `0 < ν`, `0 ≤ φ`, the predictability `φ = 0` on the closed negative half line, and
the local integrability of `φ` -- all four conditions on the **data** of `ex:hawkes`, none on the
family the recursion produces. -/
theorem strictMono_hawkesJumpTime_of_kernel (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
    (hφ0 : ∀ x, x ≤ 0 → φ x = 0)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (hxi : ∀ k, 0 < xi k) :
    StrictMono (hawkesJumpTime ν φ xi ω) :=
  strictMono_hawkesJumpTime hν hφ hφ0 hxi fun m r ↦
    intervalIntegrable_hawkesFrozen hφint _ m ω r
...
`measurable_stepPath_comp` asks of a step path exactly two things -- that the jump times be
measurable in the parameter and that the values be -- and `measurable_hawkesJumpTime` is the first
of them.  Joint measurability in `(t, ω)` is its case `γ = ℝ × Ω`, `u = Prod.fst`, exactly as in the
state dependent construction.

**Which process is measurable, and why the distinction is not pedantry.**  The statement below is
about `stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1`, and `hawkesProcess_eq_stepPath` says that
`hawkesProcess` *is* that step path -- but only at a sample point at which the waiting times are non
negative and the self referential rate is locally integrable, that is, at a sample point at which
the construction is valid.  Off that set `hawkesProcess` unfolds to `rateInverse` of a rate that is
not locally integrable, and there is no reason for it to be measurable there; the set itself is of
full measure under the driving law, by the non explosion that `ex:hawkes` proves and that this
development carries.

So the honest division is: the **step path over the Hawkes jump times** is measurable outright, with
no hypothesis beyond the data, and the **process** agrees with it exactly where it is the process.
That is the same division as in the state dependent local construction, where
`stronglyAdapted_stoppedProcess_mpFamily_jumpProcessE` is stated about the *stopped* process because
the unstopped one is not right continuous at an explosive sample point. -/

/-- **The step path over the Hawkes jump times is jointly measurable in the time and the sample
point.**  Nothing beyond the data of `ex:hawkes` is asked: `0 < ν`, `0 ≤ φ`, `φ` measurable, and
every translate of `φ` locally integrable. -/
theorem measurable_uncurry_hawkesStepPath [MeasurableSpace E] (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
    (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) :
    Measurable fun q : ℝ × ((ℕ → E) × (ℕ → ℝ)) ↦
      stepPath (hawkesJumpTime ν φ q.2.2 q.2) q.2.1 q.1 := by
  refine measurable_stepPath_comp measurable_fst (fun n ↦ ?_) (fun n ↦ ?_)
  · have hrw : (fun q : ℝ × ((ℕ → E) × (ℕ → ℝ)) ↦ hawkesJumpTime ν φ q.2.2 q.2 n)
        = (fun xi : ℕ → ℝ ↦ hawkesJumpTime ν φ xi (0 : ℕ → ℝ) n) ∘ fun q ↦ q.2.2 :=
      funext fun q ↦ hawkesJumpTime_sample_congr ν φ q.2.2 q.2 (0 : ℕ → ℝ) n
    rw [hrw]
    exact (measurable_hawkesJumpTime hν hφ hφm hφint (0 : ℕ → ℝ) n n le_rfl).comp
      (measurable_snd.comp measurable_snd)
  · exact (measurable_pi_apply n).comp (measurable_fst.comp measurable_snd)

/-! #### The filtration of the Hawkes process, and the two statements that make it the right one

`ex:hawkes` writes its rate against the counting measure of the past events, so the filtration it is
predictable for is the one that counting measure generates.  That is `hawkesFiltration`, the
`pointFiltration` of the Hawkes jump times and the chain, and `isStoppingTime_hawkesJumpTime` says
what a point process filtration is for.

**What makes this a finding and not a definition** are the two statements after it.
`hawkesPathFiltration_le_hawkesFiltration` says that the natural filtration of the *path* -- the
one the state dependent construction uses, and the only one available before this paragraph -- sits
**inside** the new one; and `not_isStoppingTime_hawkesJumpTime_pathFiltration` says that the
inclusion is strict where it matters: the first jump time is not a stopping time for the smaller
one.  The witness is the one of `not_isStoppingTime_min_jumpTimeE`, transferred and made cheaper --
a constant chain gives a constant path at every time, while `hawkesJumpTime_one` puts the first jump
at `ξ 0 / ν`, which the waiting times move.  Nothing about `φ` enters it at all.

So the enlargement is forced, and the two constructions part company exactly here: the state
dependent case could repair the defect **inside** the filtration of the path, because `rateSup` is a
functional of the path; the path dependent case cannot, and has to count. -/

/-- **The Hawkes jump times are measurable functions of the sample point.**  This is
`measurable_hawkesJumpTime` with the sample point erased by `hawkesJumpTime_sample_congr`, exactly
as in `measurable_uncurry_hawkesStepPath`. -/
theorem measurable_hawkesJumpTime_apply [MeasurableSpace E] (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
    (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (n : ℕ) :
    Measurable fun ω : (ℕ → E) × (ℕ → ℝ) ↦ hawkesJumpTime ν φ ω.2 ω n := by
  have hrw : (fun ω : (ℕ → E) × (ℕ → ℝ) ↦ hawkesJumpTime ν φ ω.2 ω n)
      = (fun xi : ℕ → ℝ ↦ hawkesJumpTime ν φ xi (0 : ℕ → ℝ) n) ∘ fun ω ↦ ω.2 :=
    funext fun ω ↦ hawkesJumpTime_sample_congr ν φ ω.2 ω (0 : ℕ → ℝ) n
  rw [hrw]
  exact (measurable_hawkesJumpTime hν hφ hφm hφint (0 : ℕ → ℝ) n n le_rfl).comp measurable_snd

/-- **The filtration of the Hawkes process**: the one generated by the counting record of its jumps
together with the path.  Nothing beyond the data of `ex:hawkes` is asked. -/
noncomputable def hawkesFiltration [MeasurableSpace E] (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
    (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) :
    Filtration ℝ≥0 (inferInstance : MeasurableSpace ((ℕ → E) × (ℕ → ℝ))) :=
  pointFiltration (fun ω : (ℕ → E) × (ℕ → ℝ) ↦ hawkesJumpTime ν φ ω.2 ω) (fun ω ↦ ω.1)
    (measurable_hawkesJumpTime_apply hν hφ hφm hφint)
    fun n ↦ (measurable_pi_apply n).comp measurable_fst

/-- **The Hawkes jump times are stopping times.**  No condition on `φ` beyond the data, and none of
non explosion: the record counts the jumps whether or not they accumulate. -/
theorem isStoppingTime_hawkesJumpTime [MeasurableSpace E] (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
    (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (n : ℕ) :
    IsStoppingTime (hawkesFiltration (E := E) hν hφ hφm hφint)
      fun ω ↦ ENNReal.ofReal (hawkesJumpTime ν φ ω.2 ω n) :=
  isStoppingTime_jumpTime _ _ n

/-- **The step path over the Hawkes jump times is adapted to the Hawkes filtration.**  Together with
`hawkesProcess_eq_stepPath` this is the adaptedness of the process at every sample point at which
the construction is valid; the division is the one explained at
`measurable_uncurry_hawkesStepPath`. -/
theorem measurable_hawkesStepPath_hawkesFiltration [MeasurableSpace E] (hν : 0 < ν)
...
noncomputable def hawkesPathFiltration [MeasurableSpace E] (hν : 0 < ν) (hφ : ∀ x, 0 ≤ φ x)
  ...
  naturalFiltration
    (fun i : ℝ≥0 ↦ fun ω : (ℕ → E) × (ℕ → ℝ) ↦
      stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1 (i : ℝ))
...
The enlargement therefore loses nothing: everything adapted to the path is adapted to the record as
well. -/
theorem hawkesPathFiltration_le_hawkesFiltration [MeasurableSpace E] (hν : 0 < ν)
    (hφ : ∀ x, 0 ≤ φ x) (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (i : ℝ≥0) :
    hawkesPathFiltration (E := E) hν hφ hφm hφint i ≤ hawkesFiltration hν hφ hφm hφint i := by
  refine iSup₂_le fun j hj ↦ ?_
  rintro A ⟨u, hu, rfl⟩
  exact (measurable_hawkesStepPath_hawkesFiltration hν hφ hφm hφint j).mono
    ((hawkesFiltration (E := E) hν hφ hφm hφint).mono hj) le_rfl hu

/-- **The Hawkes jump times are not stopping times for the filtration of the path.**  The inclusion
of `hawkesPathFiltration_le_hawkesFiltration` is therefore strict at the only place that matters,
and the counting record is not a convenience.

The witness is the one of `not_isStoppingTime_min_jumpTimeE` and it is cheaper here, because
`hawkesJumpTime_one` is unconditional in `φ`: the constant chain at `x₀` with the constant waiting
times `ν / 4` and `ν` has first jump times `1 / 4` and `1`, which the threshold `1 / 2` separates,
while both paths are constantly `x₀` at **every** time.  Both waiting times are strictly positive,
so neither sample point is degenerate.  Nothing about `φ` enters. -/
theorem not_isStoppingTime_hawkesJumpTime_pathFiltration [MeasurableSpace E] (hν : 0 < ν)
    (hφ : ∀ x, 0 ≤ φ x) (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r) (x₀ : E) :
    ¬ IsStoppingTime (hawkesPathFiltration (E := E) hν hφ hφm hφint)
      fun ω ↦ ENNReal.ofReal (hawkesJumpTime ν φ ω.2 ω 1) := by
  classical
  intro hst
  set i₀ : ℝ≥0 := Real.toNNReal (1 / 2) with hi₀
  set S : Set ((ℕ → E) × (ℕ → ℝ)) :=
    {ω | ENNReal.ofReal (hawkesJumpTime ν φ ω.2 ω 1) ≤ ((i₀ : ℝ≥0) : ENNReal)} with hS
  have hSm : MeasurableSet[hawkesPathFiltration hν hφ hφm hφint i₀] S := hst i₀
  have hcoe : ((i₀ : ℝ≥0) : ENNReal) = ENNReal.ofReal (1 / 2) := rfl
  set ω₁ : (ℕ → E) × (ℕ → ℝ) := (fun _ ↦ x₀, fun _ ↦ ν / 4) with hω₁
  set ω₂ : (ℕ → E) × (ℕ → ℝ) := (fun _ ↦ x₀, fun _ ↦ ν) with hω₂
  have h₁ : hawkesJumpTime ν φ ω₁.2 ω₁ 1 = 1 / 4 := by
    rw [hω₁, hawkesJumpTime_one hν (by positivity)]
    field_simp
  have h₂ : hawkesJumpTime ν φ ω₂.2 ω₂ 1 = 1 := by
    rw [hω₂, hawkesJumpTime_one hν hν.le]
  ...
  have hmem₂ : ω₂ ∉ S := by
    rw [hS, Set.mem_ofPred_eq, h₂, hcoe,
      ENNReal.ofReal_le_ofReal_iff (by norm_num : (0 : ℝ) ≤ 1 / 2)]
    norm_num
  have hpath : ∀ r : ℝ≥0, r ≤ i₀ →
      (fun ω : (ℕ → E) × (ℕ → ℝ) ↦ stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1 (r : ℝ)) ω₁
        = (fun ω : (ℕ → E) × (ℕ → ℝ) ↦ stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1 (r : ℝ)) ω₂ :=
    fun r _ ↦ rfl
  have hGm : Measurable[hawkesPathFiltration hν hφ hφm hφint i₀] (S.indicator fun _ ↦ (1 : ℝ)) :=
    measurable_const.indicator hSm
  have hsep := eq_of_measurable_naturalFiltration
    (X := fun i : ℝ≥0 ↦ fun ω : (ℕ → E) × (ℕ → ℝ) ↦
      stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1 (i : ℝ)) _ hGm hpath
  rw [Set.indicator_of_mem hmem₁, Set.indicator_of_notMem hmem₂] at hsep
  exact one_ne_zero hsep
...
/-- **A compensating window whose upper end *is* the time parameter is jointly measurable**, as
soon as its integrand is.  `measurable_compensator_of_uncurry_min` is the same statement at a
*fixed* window and gives measurability in the sample point alone; a test process is progressively
measurable only when the window moves with the time, and that is this.

The proof is the piece of set algebra the assembly of the state dependent case already used: a
window with a **variable** upper end is a window of **fixed** length with a cut integrand, and the
cut set `{(u, p) | u ∈ Q.interval c ⊥ (min p.1 t)}` is measurable because it constrains the
integration variable and the time parameter only -- never the sample point.

**No continuity of the primitive and no bound on the integrand enter**, and that is the whole
difference to `isStronglyProgressive_mpFamily_jumpProcessE`, which had to reach the same conclusion
through the right continuity of the compensator and the dyadic approximation of
`measurable_uncurry_min_of_rightContinuous`. -/
theorem measurable_uncurry_compensator_of_uncurry_min {mΩ : MeasurableSpace Ω} {W : ℝ → Ω → ℝ}
    (c : Clock.Conv) (t : ℝ≥0)
    (hW : Measurable[(inferInstance : MeasurableSpace ℝ≥0).prod mΩ]
      fun p : ℝ≥0 × Ω ↦ W (min (p.1 : ℝ) (t : ℝ)) p.2) :
    Measurable[(inferInstance : MeasurableSpace ℝ≥0).prod mΩ]
      fun p : ℝ≥0 × Ω ↦ ∫ u in lebesgueClock.interval c ⊥ (min p.1 t),
        W (u : ℝ) p.2 ∂lebesgueClock.q := by
  set m : MeasurableSpace (ℝ≥0 × Ω) := (inferInstance : MeasurableSpace ℝ≥0).prod mΩ with hm
  set S := lebesgueClock.interval c ⊥ t with hS
  have hfin : IsFiniteMeasure (lebesgueClock.q.restrict S) := by
    refine ⟨?_⟩
    rw [Measure.restrict_apply_univ]
    exact lt_top_iff_ne_top.2 (ne_top_of_le_ne_top (lebesgueClock.measure_Iic_ne_top t)
      (measure_mono (lebesgueClock.interval_subset_Iic c ⊥ t)))
  -- a shorter window sits inside the fixed one
  have hsub : ∀ p : ℝ≥0 × Ω, lebesgueClock.interval c ⊥ (min p.1 t) ⊆ S := by
    intro p
    rw [hS, (lebesgueClock.interval_union c (bot_le : (⊥ : ℝ≥0) ≤ min p.1 t)
      (min_le_right p.1 t)).1]
    exact Set.subset_union_left
  -- a variable upper end is a fixed window with a cut integrand
  have hrw : (fun p : ℝ≥0 × Ω ↦ ∫ u in lebesgueClock.interval c ⊥ (min p.1 t),
        W (u : ℝ) p.2 ∂lebesgueClock.q)
      = fun p : ℝ≥0 × Ω ↦ ∫ u in S, Set.indicator (lebesgueClock.interval c ⊥ (min p.1 t))
          (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) p.2) u ∂lebesgueClock.q := by
    funext p
    rw [setIntegral_indicator (lebesgueClock.measurableSet_interval c ⊥ (min p.1 t)),
      Set.inter_eq_self_of_subset_right (hsub p)]
    refine setIntegral_congr_fun (lebesgueClock.measurableSet_interval c ⊥ (min p.1 t))
      fun u hu ↦ ?_
    have hut : u ≤ t :=
      (lebesgueClock.interval_subset_Iic c ⊥ (min p.1 t) hu).trans (min_le_right p.1 t)
    have hmin : min (u : ℝ) (t : ℝ) = (u : ℝ) := min_eq_left (by exact_mod_cast hut)
    rw [hmin]
  have hjoint : Measurable[(inferInstance : MeasurableSpace ℝ≥0).prod m]
      (Function.uncurry fun (u : ℝ≥0) (p : ℝ≥0 × Ω) ↦
        Set.indicator (lebesgueClock.interval c ⊥ (min p.1 t))
          (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) p.2) u) := by
    letI : MeasurableSpace (ℝ≥0 × Ω) := m
    have h1 : Measurable fun q : ℝ≥0 × (ℝ≥0 × Ω) ↦ q.1 := measurable_fst
    have h2 : Measurable fun q : ℝ≥0 × (ℝ≥0 × Ω) ↦ min q.2.1 t :=
      (measurable_fst.comp measurable_snd).min measurable_const
    have hD : MeasurableSet
        {q : ℝ≥0 × (ℝ≥0 × Ω) | q.1 ∈ lebesgueClock.interval c ⊥ (min q.2.1 t)} := by
      cases c with
      | optional => exact (measurableSet_le h1 h2).diff (measurableSet_le h1 measurable_const)
      | predictable => exact (measurableSet_lt h1 h2).diff (measurableSet_lt h1 measurable_const)
    have hfun : (Function.uncurry fun (u : ℝ≥0) (p : ℝ≥0 × Ω) ↦
          Set.indicator (lebesgueClock.interval c ⊥ (min p.1 t))
            (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) p.2) u)
        = Set.indicator {q : ℝ≥0 × (ℝ≥0 × Ω) | q.1 ∈ lebesgueClock.interval c ⊥ (min q.2.1 t)}
            (fun q : ℝ≥0 × (ℝ≥0 × Ω) ↦ W (min (q.1 : ℝ) (t : ℝ)) q.2.2) := by
      funext q
      by_cases hq : q.1 ∈ lebesgueClock.interval c ⊥ (min q.2.1 t)
      · rw [Set.indicator_of_mem (show q ∈ {q : ℝ≥0 × (ℝ≥0 × Ω) |
          q.1 ∈ lebesgueClock.interval c ⊥ (min q.2.1 t)} from hq)]
        exact Set.indicator_of_mem hq (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) q.2.2)
      · rw [Set.indicator_of_notMem (show q ∉ {q : ℝ≥0 × (ℝ≥0 × Ω) |
          q.1 ∈ lebesgueClock.interval c ⊥ (min q.2.1 t)} from hq)]
        exact Set.indicator_of_notMem hq (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) q.2.2)
    rw [hfun]
    exact (hW.comp (measurable_fst.prodMk (measurable_snd.comp measurable_snd))).indicator hD
  have hsm : StronglyMeasurable[m] fun p : ℝ≥0 × Ω ↦
      ∫ u in S, Set.indicator (lebesgueClock.interval c ⊥ (min p.1 t))
        (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) p.2) u ∂lebesgueClock.q :=
    @stronglyMeasurable_integral_comp ℝ≥0 lebesgueClock.measurableSpace (ℝ≥0 × Ω) m ℝ _ ℝ _
      (lebesgueClock.q.restrict S) _
      (fun (u : ℝ≥0) (p : ℝ≥0 × Ω) ↦
        Set.indicator (lebesgueClock.interval c ⊥ (min p.1 t))
          (fun v : ℝ≥0 ↦ W (min (v : ℝ) (t : ℝ)) p.2) u) hjoint id measurable_id
  rw [hrw]
  exact hsm.measurable

end PointRate

/-! ### The test processes of the path dependent problem are progressively measurable

The one statement that turns the measurability layer into a statement about a *process*, and the
place where the path dependent variant turns out to be **cheaper** than the state dependent one.
`isStronglyProgressive_mpFamily_jumpProcessE` had to spend the right continuity of the compensator
and the dyadic approximation on it, and it had to carry a bound on the rate for that; here the
joint measurability of the integrand is the hypothesis, and the step from it to the moving window
is `measurable_uncurry_compensator_of_uncurry_min`, which is set algebra.

The reason for the difference is not a better proof.  It is that `mpFamilyF` asks its hypotheses of
the **integrand** and `mpFamily` asks them of the **process**: a hypothesis about the integrand is
already the joint measurability the parametrised Bochner integral wants, while a hypothesis about
the process has to be turned into one about the integrand first, and that turning is the dyadic
argument. -/

section PathProgressive

variable {Ω : Type*} {mΩ : MeasurableSpace Ω} {E : Type*} [MeasurableSpace E]

omit [MeasurableSpace E] in
/-- **The test processes of a path dependent martingale problem are strongly progressive**, as soon
as the process and the compensating integrand are jointly measurable for the filtration in the
truncated form `Clock.IsProgressive` supplies.

Nothing is asked of the rate, of the kernel, or of the sample space: the statement is about an
arbitrary decomposition `Y s ω = f (X s ω) - ∫ u in Q.interval c ⊥ s, W u ω`, which is what
membership in `mpFamilyF` gives.  In particular **no bound, no continuity and no non explosion**
enter. -/
theorem isStronglyProgressive_of_measurable_uncurry_mpFamilyF {𝓕 : Filtration ℝ≥0 mΩ}
    {c : Clock.Conv} {f : E → ℝ} {X : ℝ≥0 → Ω → E} {W : ℝ → Ω → ℝ} {Y : ℝ≥0 → Ω → ℝ}
    (hYeq : ∀ s ω, Y s ω
      = f (X s ω) - ∫ u in lebesgueClock.interval c ⊥ s, W (u : ℝ) ω ∂lebesgueClock.q)
    (hX : ∀ t : ℝ≥0, Measurable[(inferInstance : MeasurableSpace ℝ≥0).prod (𝓕 t)]
      fun p : ℝ≥0 × Ω ↦ f (X (min p.1 t) p.2))
    (hW : ∀ t : ℝ≥0, Measurable[(inferInstance : MeasurableSpace ℝ≥0).prod (𝓕 t)]
      fun p : ℝ≥0 × Ω ↦ W (min (p.1 : ℝ) (t : ℝ)) p.2) :
    IsStronglyProgressive 𝓕 Y := by
  refine isStronglyProgressive_of_measurable_uncurry_min fun t ↦ ?_
  have heq : (fun p : ℝ≥0 × Ω ↦ Y (min p.1 t) p.2)
      = fun p : ℝ≥0 × Ω ↦ f (X (min p.1 t) p.2)
        - ∫ u in lebesgueClock.interval c ⊥ (min p.1 t), W (u : ℝ) p.2 ∂lebesgueClock.q :=
    funext fun p ↦ hYeq _ _
  rw [heq]
  exact (hX t).sub (measurable_uncurry_compensator_of_uncurry_min c t (hW t))
...
/-- **The test processes of the Hawkes martingale problem are strongly progressive.**  The first of
the two inputs `martingale_stoppedProcess` asks of a test process, and the statement the whole
measurability layer of the path dependent variant was built for.

The process is the **step path over the Hawkes jump times** and not `hawkesProcess`, and that is
deliberate: `hawkesProcess_eq_stepPath` identifies the two exactly where the construction is valid,
and every hypothesis it carries -- the non negativity of the waiting times, the interval
integrability of the frozen and of the self referential rate -- is a hypothesis this statement does
not need.  Nothing beyond the data of `ex:hawkes` and the Markov property of the jump kernel
enters, **and in particular not the non explosion**.

The jump kernel is a `Kernel E E` and reads the state alone, which is the case of `ex:hawkes`:
there only the *rate* is self exciting.  A kernel that reads the past as well is a further
generalisation of `jumpApplyF`, and it would need its own joint measurability in place of
`measurable_integral_sub_kernel`; nothing else in the proof would change. -/
theorem isStronglyProgressive_mpFamilyF_hawkesStepPath [MeasurableSpace E] (hν : 0 < ν)
    (hφ : ∀ x, 0 ≤ φ x) (hφm : Measurable φ)
    (hφint : ∀ a r : ℝ, IntervalIntegrable (fun u ↦ φ (u - a)) volume 0 r)
    (mu : Kernel E E) [IsMarkovKernel mu] (c : Clock.Conv)
    {Y : ℝ≥0 → ((ℕ → E) × (ℕ → ℝ)) → ℝ}
    (hY : Y ∈ mpFamilyF
      ...
      (fun (s : ℝ≥0) (ω : (ℕ → E) × (ℕ → ℝ)) ↦
        stepPath (hawkesJumpTime ν φ ω.2 ω) ω.1 (s : ℝ))) :
    IsStronglyProgressive (hawkesFiltration (E := E) hν hφ hφm hφint) Y := by
  obtain ⟨p, ⟨hf, ⟨C, hC⟩, hp2⟩, hYeq⟩ := hY
  ...
  · rw [hYeq s ω, hp2]
    rfl
  · have h := measurable_uncurry_hawkesStepPath_hawkesFiltration hν hφ hφm hφint hf t
    simpa only [NNReal.coe_min] using h
