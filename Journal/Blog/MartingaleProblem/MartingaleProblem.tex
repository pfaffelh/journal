...
\begin{proof}
  ...
  \]
  is bounded and $P X^{-1}$-a.s.\ continuous on $\DE$.  For the integral term
  this needs an extra word: if $\omega_m \to \omega$ in $\DE$, then
  $\omega_m(u) \to \omega(u)$ for every $u$ outside the at most countable set of
  discontinuities of $\omega$, so $g(\omega_m(u)) \to g(\omega(u))$ for Lebesgue
  almost every $u$ by continuity of $g$, and bounded convergence gives
  $\int_t^s g(\omega_m(u))\dif u \to \int_t^s g(\omega(u))\dif u$ --- for
  \emph{every} $\omega$, so this summand is in fact continuous everywhere.  By
  the continuous mapping theorem (Fact~\ref{fact:cmt}) and \eqref{eq:approxA} ---
  which bounds the difference between the two integrands by
  $2\lVert f_n - f\rVert + (s-t)\lVert g_n - g\rVert$ times
  $\prod_i \lVert h_i \rVert$, uniformly in $\omega$ ---
  \begin{align}
    \label{eq:convlimit}
  ...
  Proposition~\ref{prop:fddchar}.  For general
  $0 \leq t_1 \leq \dots \leq t_k \leq t < s$ choose
  $t^m_i \downarrow t_i$, $t^m \downarrow t$ and $s^m \downarrow s$ in $D(X)$ with
  $t^m_i \leq t^m < s^m$, which is possible because $D(X)$ is dense; the
  integrand is bounded by
  $(2\lVert f\rVert + (s^1-t)\lVert g\rVert)\prod_i\lVert h_i\rVert$ and converges
  pointwise by right continuity of the paths of $X$, so bounded convergence
  extends \eqref{eq:convlimit} to all such times.  By
