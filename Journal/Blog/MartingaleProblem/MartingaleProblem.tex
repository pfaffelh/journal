...
\item \textbf{Abstract working layer: also (A).}
  ...
  Proposition~\ref{prop:haar} for duality.  This layer is itself graded: a
...
\begin{proof}
  By Proposition~\ref{prop:fddchar} and the linearity of \eqref{eq:fdd} in
  $(f,g)$, the identity \eqref{eq:fdd} holds for every $(f_n,g_n) \in A_S$.  The
  test factor $\prod_{k} h_k(X(t_k))$ is bounded by $\prod_k \lVert h_k \rVert$,
  so each of the two convergences in \eqref{eq:closureconv} passes through the
  expectation in \eqref{eq:fdd}.  Hence \eqref{eq:fdd} holds for $(f,g)$, and
  Proposition~\ref{prop:fddchar} applies again.
\end{proof}
...
\begin{proof}
  ...
  dominated convergence under $q \otimes P$.  Both dominating constants depend on
  the sequence alone.
\end{proof}
...
\begin{definition}[Propagation of agreement; (T0) + (E0)]
  ...
  $P, Q \in \mathcal{N}$ and all $s \leq t$ in $\T$,
  \begin{equation}
...
\begin{proof}
  Let $P, Q \in \mathcal{N}$ with $P\pi_0^{-1} = Q\pi_0^{-1}$.  We show by induction on $m$ that
  $P = Q$ on $\Filt^\circ_{t_m}$ for every chain $t_1 \leq \dots \leq t_m$ in
  $\T$; since $\mathcal{S} = \sigma(\pi_t : t \in \T)$ by \eqref{eq:pathsigma},
  this gives $P = Q$.

  For $m = 0$ the assertion is $P = Q$ on $\Filt^\circ_0 = \sigma(\pi_0)$, which
  is the hypothesis on the initial laws.  Assume $P = Q$ on
  $\Filt^\circ_{t_n}$ and let $t_{n+1} \geq t_n$.  Then \eqref{eq:propagation}
  gives $P = Q$ on $\Filt^\circ_{t_n} \vee \sigma(\pi_{t_{n+1}})$, and since the
  products $\prod_{k \leq n+1} h_k(\pi_{t_k})$ with $t_k \leq t_{n+1}$ form a
  multiplicative class generating $\Filt^\circ_{t_{n+1}}$, the Dynkin argument
  upgrades this to $P = Q$ on $\Filt^\circ_{t_{n+1}}$.
\end{proof}
...
\begin{proof}
  Let $P = Q$ on $\Filt^\circ_s$, let $G \in \Filt^\circ_s$ with $P(G) > 0$ and
  put $Z = \one_G / P(G)$, which is nonnegative and $\Filt^\circ_s$-measurable
  with the same mean $1$ under $P$ and under $Q$.  By Lemma~\ref{lem:restart},
  $R_P = (Z \cdot P)\circ\theta_s^{-1}$ and $R_Q = (Z \cdot Q)\circ\theta_s^{-1}$
  both lie in $\Msol(\XX^\circ_s)$ --- the \emph{same} shifted problem.  Since
  $\pi_0 \circ \theta_s = \pi_s$, their initial laws are
  $P\{\pi_s \in \cdot \mid G\}$ and $Q\{\pi_s \in \cdot \mid G\}$, which agree
  because $P = Q$ on $\Filt^\circ_s$.  So \eqref{eq:absonedim} applies and gives
  $E^{R_P}[f(\pi_u)] = E^{R_Q}[f(\pi_u)]$ for every $u$; taking $u$ with
  $s + u = t$ this reads
  $E^P[f(\pi_t)\one_G] = E^Q[f(\pi_t)\one_G]$.  As $G$ and $f$ were arbitrary,
  $P = Q$ on $\Filt^\circ_s \vee \sigma(\pi_t)$.
\end{proof}
...
\begin{remark}[Uniqueness is not a Markovian fact]
  ...
  \begin{itemize}
  \item \emph{Path-dependent equations.}  Pathwise uniqueness implies uniqueness
    in law (Fact~\ref{fact:yamadawatanabe}), and uniqueness in law
    \emph{is} \eqref{eq:propagation}.  The coefficients may depend on the whole
    past.
...
\begin{proof}
  ...
  one, hence nonnegative and $\Filt^\circ_r$-measurable, so
...
\begin{example}[Volterra equations; \CPS, Example 3.13]
  ...
  \item \textbf{Proposition~\ref{prop:uniqfromprop} applies}: whenever the
    Volterra equation has a unique solution in law --- which under Lipschitz
    conditions on $b,a,\nu$ it does --- that uniqueness in law \emph{is}
    \eqref{eq:propagation}, and Proposition~\ref{prop:uniqfromprop} turns it into
    uniqueness of the whole law without any Markov structure;
  ...
  \item Theorem~\ref{thm:absuniq} does \emph{not} apply, and the conclusion is
    ...
    Theorem~\ref{thm:absstrongmarkov}, Section~\ref{ssec:localuniq} or
    Section~\ref{sec:duality}, all of which use the shift.
  \end{itemize}
\end{example}
...
\begin{example}[Semimartingales with path-dependent characteristics]
  ...
  while Section~\ref{ssec:localuniq} is not, since pasting is a shift operation
  (Remark~\ref{rem:localuniqcomment}).  This is the sharpest illustration of the
  split: \emph{localization is not Markovian, restarting is.}
\end{example}
...
\begin{remark}[Where the Markov structure is used, in one place]
  ...
  Lemma~\ref{lem:chain} (chain identity)
    & Section~\ref{ssec:localuniq} (local uniqueness) \\
...
\begin{example}[Rescaled Markov chains; the invariance principle]
  ...
  approximant
  \[
    q^n = \frac{1}{n} \sum_{k \geq 0} \delta_{k/n} , \qquad \iota_n = \mathrm{p} .
  \]
  ...
  With $\xi^n = f(X^n)$ and $\psi^n_u = n\,(P_n f - f)(X^n_u)$ we get
  \[
    \int_{[0,t)} \psi^n_u \, q^n(\dif u)
      = \frac{1}{n} \sum_{k < nt} \psi^n_{k/n}
      = \sum_{k < nt} (P_n f - f)(\Xi^n_k) ,
  \]
  so that \eqref{eq:approxclock} is exactly the Doob decomposition of
  $f(\Xi^n_k)$ and is a martingale --- this is Remark~\ref{rem:jumpdiscrete}, and
  it is where $\iota_n = \mathrm{p}$ is used.  Then \ref{it:K2} holds trivially,
  $\xi^n - f(X^n)$ being identically zero; \ref{it:K3} is the classical
  requirement that the rescaled generators $n(P_nf - f)$ converge to $Af$; and
  \ref{it:K4} holds because $\psi^n$ is constant on each interval
  $[k/n,(k+1)/n)$, so that the discrete and the Lebesgue integral agree except on
  the two boundary intervals, giving
  $|\Delta^n_{s,t}| \leq \tfrac{2}{n} \sup_u |\psi^n_u|$, which tends to $0$ as
  soon as $n(P_nf-f)$ is bounded.
...
\begin{remark}[What this example costs, and what paid for it]
  ...
  \item \emph{Both conventions} (Remark~\ref{rem:convention}).  The chain needs
    $\iota = \mathrm{p}$, the limit does not care.  Had one convention been fixed,
    half of Example~\ref{ex:invariance} would have to be rewritten; as it is, the
    two differ by $\psi^n_t \, q^n(\{t\}) = (P_nf-f)(X^n_t) = O(1/n)$ and
    \emph{wash out in the limit}.  This is a retrospective argument for carrying
...
% =====================================================================
\subsection{Weak-strong convergence: relaxing the continuity hypothesis}
...
\begin{example}[Why \ref{it:C3a} is not a technicality; \CPS, Section 4.3]
  ...
  Take $\T = \Rp$, $E = \R$, $F = D_\R[0,\infty)$ and the atomic clock
  $q = \delta_1$.  The compensator of Definition~\ref{def:markovMP} is then the
  functional
  \[
    F_t(\omega) \;=\; \int_{\langle 0,t \rangle_{\mathrm{p}}} \omega(s) \, q(\dif s)
      \;=\;
      \begin{cases} 0, & t < 1, \\ \omega(1-), & t \geq 1 . \end{cases}
  \]
  For $t < 1$ the map $\omega \mapsto F_t(\omega)$ is $J_1$-continuous.  For
  \emph{every} $t \geq 1$ it is discontinuous: take
  $\omega_n = \one_{[1-1/n,\infty)} \to \omega = \one_{[1,\infty)}$ in $J_1$, and
  observe $F_t(\omega_n) = 1 \not\to 0 = F_t(\omega)$.

  So there is no dense $\Gamma \subset \Rp$ on which $F_t$ is continuous, and
  hypothesis \ref{it:C3a} of Theorem~\ref{thm:absconv} fails for a set of times of
  full measure --- not, as in Theorem~\ref{thm:CPSconv}, on a countable set that a
  dense $\Gamma$ can avoid.  The cause is exactly the atom of $q$, and hence
  exactly the generality that Remark~\ref{rem:convention} and
...
\begin{remark}[What the control variable does]
  ...
  limit has no fixed times of discontinuity.  Admitting atoms in the clock, as
  Section~\ref{ssec:jump} and Section~\ref{ssec:diffclocks} do, is exactly what
  makes the general case necessary.  Together with Remark~\ref{rem:conventionclash}
  this is the second place where atoms exact a price: there a choice of
  convention, here a strengthening of the mode of convergence.
\end{remark}
...
\begin{theorem}[Path-dependent jump processes; (T3) + (E0)]
  ...
  Suppose $\zeta = \infty$ almost surely.  Then $X$ of
  Definition~\ref{def:pathjumpconstruction} solves the martingale problem for
  $\XX^\circ$ of Setting~\ref{set:pathjump} with initial law $\nu$.  If
  $\zeta < \infty$ with positive probability, $X$ solves the local martingale
  problem on $[0,\zeta)$, with $(\tau_n)$ a localizing system in the sense of
  Definition~\ref{def:localizing}.
\end{theorem}

\begin{proof}
  The decomposition of Step~1 in the proof of Theorem~\ref{thm:jumpMP} is
  unchanged, so it suffices to show $E[D_n \mid \mathcal{H}_n] = 0$ on
  $\{\tau_n \leq t\}$ for
  \[
    D_n = \bigl( f(Y_{n+1}) - f(Y_n) \bigr)\one_{\{\tau_{n+1}\leq t\}}
  ...
  \emph{coincide}.  The rest of the proof of Theorem~\ref{thm:jumpMP} --- the
  passage from $0$ to $s$, and the localization --- goes through verbatim, the
  memorylessness of the exponential being replaced by the fact that, conditionally
  on $\Filt^\circ_s$, the residual jump time is again given by
  \eqref{eq:pathjumptime} with $\tau_n$ replaced by $s$.
\end{proof}
...
\begin{example}[Hawkes processes]
  ...
  $\lVert\phi\rVert_1 < 1$ one has $\sup_{t\leq T} E[\Lambda_t] < \infty$ and
  $\zeta = \infty$, while for $\lVert\phi\rVert_1 \geq 1$ explosion is possible and
...
\begin{theorem}[Hawkes to Volterra, conditionally on tightness; (T3) + (E3)]
  ...
  that problem has a unique solution in law for the given initial condition, then
...
\begin{proof}
  ...
  Proposition~\ref{prop:uniqfromprop}: uniqueness in law is
  \eqref{eq:propagation}, and it upgrades to uniqueness of the whole law with no
  Markov structure --- which is essential, since a Volterra process has none.
\end{proof}
...
\begin{remark}[What is proved here and what is cited]
  ...
  Three remarks on scope.  First, \emph{weak-strong convergence is not needed}:
  the jumps of $N^n$ have height one and the limit is continuous, so the limit has
  no fixed times of discontinuity and \ref{it:C3a} holds on a dense set with
  countable complement.  Section~\ref{ssec:weakstrong} stays out of it.  Second,
